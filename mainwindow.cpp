...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    setWindowTitle(tr("CPPad"));
}

void MainWindow::about()
{
    QMessageBox::about(this, tr("About CPPad"),tr("<p>The <b>CPPad</b> "));
}
...
void MainWindow::setupEditor()
{
    ...
    QFile file("/media/data/dev/qt/__build__/CPPad-debug/source.cpp");
    if (file.open(QFile::ReadOnly | QFile::Text))
        editor->setPlainText(file.readAll());
}
...
void MainWindow::createActions()
{
    ...
    runAct = new QAction(QIcon::fromTheme("system-run"), tr("&Run"), this);
    runAct->setShortcuts(QKeySequence::Bold);
    runAct->setStatusTip(tr("Build and execute the current source code"));
...
void MainWindow::run()
{
    ...
    QFile cpp("source.cpp");
    cpp.open(QIODevice::WriteOnly | QIODevice::Text);
    cpp.write(editor->toPlainText().toLocal8Bit());
    cpp.close();

    QProcess gcc;
    gcc.start("g++ -o program source.cpp");
    gcc.waitForFinished();
    int ret = gcc.exitCode();

    if(ret == 0)
    {
        statusBar()->showMessage(tr("Build success"), 2000);

        QProcess program;
        program.start("./program");
        program.waitForFinished();
        //int ret = program.exitCode();
        ui->console->setText(program.readAll());
    }
    else
    {
        //ui->console->setText(program.readAll());
